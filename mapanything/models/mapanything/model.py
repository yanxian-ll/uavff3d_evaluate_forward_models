...
from mapanything.utils.geometry import (
    ...
    normalize_pose_translation_distances,
...
class MapAnything(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        # Initialize the encoder for camera translation distance (distance-only branch)
        cam_trans_dist_encoder_config = self.geometric_input_config[
            "cam_trans_dist_encoder_config"
        ]
        cam_trans_dist_encoder_config["enc_embed_dim"] = self.encoder.enc_embed_dim
        self.cam_trans_dist_encoder = encoder_factory(**cam_trans_dist_encoder_config)
    ...
    def _compute_pose_quats_in_ref_view_and_trans_dists_for_across_views(
        self,
        views,
        num_views,
        device,
        ...
        - trans branch: distance-only, compute ||t_i - t_0|| without direction

        Returns:
            pose_quats_across_views: (B*V, 4)
            pose_trans_dist_across_views: (B*V, 1)
        ...
        pose_trans_dist_across_views = torch.zeros(
            (total_batch, 1), dtype=dtype, device=device
        )
        ...
        # ---------------------------
        # 2) translation-distance-only branch
        # ---------------------------
        for view_idx in range(num_views):
            start = view_idx * batch_size_per_view
            end = (view_idx + 1) * batch_size_per_view
            ...
            if  curr_mask.any() and  "camera_pose_trans" in views[view_idx]:
                ...
                rel_dist = torch.norm(curr_trans - ref_trans, dim=-1, keepdim=True)
                pose_trans_dist_across_views[start:end][curr_mask] = rel_dist.to(dtype=dtype)
            else:
                per_sample_cam_trans_input_mask[start:end] = False

        return (
            pose_quats_across_views,
            pose_trans_dist_across_views,
            per_sample_cam_rot_input_mask,
            per_sample_cam_trans_input_mask,
        )
    ...
    def _encode_and_fuse_split_cam_quats_and_trans_dists(
        self,
        views,
        num_views,
        batch_size_per_view,
        ...
        - trans branch: distance-only + norm_factor
        """
        ...
        # ----------------------------------
        # translation-distance branch
        # ----------------------------------
        pose_trans_dist_features_across_views = torch.zeros(
        ...
        pose_trans_scale_features_across_views = torch.zeros_like(
            pose_trans_dist_features_across_views
        )

        if per_sample_cam_trans_input_mask.any():
            pose_trans_dist = torch.split(
                pose_trans_dist_across_views, batch_size_per_view, dim=0
            )
            pose_trans_dist = torch.stack(pose_trans_dist, dim=1)  # (B, V, 1)
            scaled_pose_trans_dist, pose_trans_norm_factors = (
                normalize_pose_translation_distances(
                    pose_trans_dist, return_norm_factor=True
                )
            )

            scaled_pose_trans_dist = torch.cat(
                scaled_pose_trans_dist.unbind(dim=1), dim=0
            )  # (B*V, 1)
            ...
            trans_dist_feats = self.cam_trans_dist_encoder(
                EncoderGlobalRepInput(data=scaled_pose_trans_dist)
            ).features
            trans_dist_feats = (
                trans_dist_feats * per_sample_cam_trans_input_mask.unsqueeze(-1)
            )
            pose_trans_dist_features_across_views = (
                pose_trans_dist_features_across_views + trans_dist_feats
            )
        ...
        all_encoder_features_across_views = (
            all_encoder_features_across_views
            + pose_quats_features_across_views.unsqueeze(-1).unsqueeze(-1)
            + pose_trans_dist_features_across_views.unsqueeze(-1).unsqueeze(-1)
            + pose_trans_scale_features_across_views.unsqueeze(-1).unsqueeze(-1)
        )
    ...
    def _encode_and_fuse_optional_geometric_inputs(
        ...
        if "cam_rot_prob" in self.geometric_input_config or "cam_trans_prob" in self.geometric_input_config:
            ...
            pose_quats_across_views, pose_trans_dist_across_views, per_sample_cam_rot_input_mask, per_sample_cam_trans_input_mask = (
                self._compute_pose_quats_in_ref_view_and_trans_dists_for_across_views(
                    views,
                    num_views,
                    device,
                    dtype,
            ...
            all_encoder_features_across_views = (
                self._encode_and_fuse_split_cam_quats_and_trans_dists(
                    views,
                    num_views,
                    batch_size_per_view,
                    all_encoder_features_across_views,
