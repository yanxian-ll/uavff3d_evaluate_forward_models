...
class DPT(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        feats = [rearrange(feat[0], "B S N C -> (S B) N C") for feat in feats]

        # update image info, used by the GS-DPT head
        extra_kwargs = {}
        if "images" in kwargs:
            # extra_kwargs.update({"images": rearrange(kwargs["images"], "B S ... -> (B S) ...")})
            extra_kwargs["images"] = rearrange(kwargs["images"], "B S ... -> (S B) ...")
        ...
        if chunk_size is None or chunk_size >= S:
            ...
            out_dict = {k: rearrange(v, "(S B) ... -> B S ...", B=B, S=S) for k, v in out_dict.items()}
            return Dict(out_dict)
