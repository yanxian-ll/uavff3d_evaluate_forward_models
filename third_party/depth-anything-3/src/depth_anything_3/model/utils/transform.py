...
def extri_intri_to_pose_encoding(
    ...
    """
    Convert camera extrinsics and/or intrinsics to a compact 9D pose encoding.
    ...
    Missing fields are zero-filled so the encoding dimensionality stays fixed and
    pretrained CameraEnc weights remain compatible.
    """
    ref = extrinsics if extrinsics is not None else intrinsics
    leading_shape, dtype, device = ref.shape[:-2], ref.dtype, ref.device

    T = torch.zeros(*leading_shape, 3, dtype=dtype, device=device)
    quat = torch.zeros(*leading_shape, 4, dtype=dtype, device=device)
    fov_h = torch.zeros(*leading_shape, dtype=dtype, device=device)
    fov_w = torch.zeros(*leading_shape, dtype=dtype, device=device)
