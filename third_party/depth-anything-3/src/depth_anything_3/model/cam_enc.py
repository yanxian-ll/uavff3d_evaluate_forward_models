...
class CameraEnc(nn.Module):
    ...
    def forward(
        ...
        """
        Args:
            ext: [B, S, 4, 4] or [B, S, 3, 4] world-to-camera extrinsics, optional.
            ixt: [B, S, 3, 3] intrinsics, optional.
            image_size: (H, W). Required when ixt is not None.
        """

        if ext is None and ixt is None:
            raise ValueError("CameraEnc.forward requires at least one of ext or ixt.")

        c2ws = affine_inverse(ext) if ext is not None else None
