...
def is_scene_dir(scene_dir: Path, images_dir: str, cams_dir: str, depth_dir: str) -> bool:
    return (
        scene_dir.is_dir()
        and (scene_dir / images_dir).is_dir()
        and (scene_dir / cams_dir).is_dir()
        and (scene_dir / depth_dir).is_dir()
    )
...
def find_scene_dirs(
    ...
    for p in sorted(candidates):
        if not is_scene_dir(p, images_dir, cams_dir, depth_dir):
            continue
...
def export_one_scene(scene_root: Path, save_rrd: Path, args: argparse.Namespace) -> None:
    ...
    print(f"  images={len(images)} cams={len(cam_files)} depths={len(depths)} common={len(common_stems)}")
    ...
    print(f"  frames used for point cloud: {len(selected_stems)} / {len(common_stems)}")
...
def main() -> None:
    ...
    if not scenes:
        raise RuntimeError(
            ...
            f"Expected each scene to contain {args.images_dir}/, {args.cams_dir}/, {args.depth_dir}/."
        )
